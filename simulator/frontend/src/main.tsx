...
function App(){
 ...
 const disabled=busy||commandBusy||running||!connected||state?.hardware?.mode==='SERVICE'||state?.hardware?.mode==='ARMING';
 ...
 useEffect(()=>{
  const key=(e:KeyboardEvent)=>{
   ...
   if(e.key==='['||e.key===']'){
    e.preventDefault();const limit=(latest.state?.effective_limits_deg??boot?.model.limits_deg)?.[selectedJoint];if(!limit)return;
  ...
  <div className="workspace">
   <section className="scene-stage" data-model-status={modelStatus} data-path-visible={Boolean(shownPlan)} aria-label="机械臂工作区">
    ...
    {drawer&&drawer!=='console'&&<section className={'drawer '+(drawer==='actions'?'sequence-drawer':'')+(drawer==='hardware'?' hardware-drawer':'')} role="dialog" aria-label={drawerTitles[drawer]}>
     ...
     {drawer==='diagnostics'&&<div className="diagnostics-panel">
      ...
      <h3>模型关节范围 <span>软件软限位</span></h3><div className="limit-list">{(state.hardware?.model_limits_deg??boot.model.limits_deg).map((limit,index)=><div key={index}><b>{index===5?'G':'J'+(index+1)}</b><span>{names[index]}</span><code>{limit[0]}° ～ {limit[1]}°</code></div>)}</div>
     ...
     {drawer==='hardware'&&<div className="hardware-panel">
      ...
      {hardwareSection==='servos'&&<div className="debug-section">{state.hardware?.connected?<CalibrationWorkbench key={state.hardware.device_id} hardware={state.hardware} model={boot.model} modelLimits={state.hardware.model_limits_deg??boot.model.limits_deg} onState={receiveHardware}/>:<p>请先连接 ESP32-S3</p>}</div>}
      ...
      {hardwareSection==='communication'&&state.hardware?.wiring&&<div className="debug-section"><h3>固件接线配置</h3><p>SDA → GPIO{state.hardware.wiring.i2c.sda_gpio} · SCL → GPIO{state.hardware.wiring.i2c.scl_gpio} · I2C {state.hardware.wiring.i2c.clock_hz} Hz</p><p>PCA9685 地址 0x{state.hardware.wiring.pca9685.address.toString(16)} · PWM {state.hardware.wiring.pca9685.pwm_hz} Hz</p><p>{state.hardware.wiring.axes.map(a=>`${a.name} → PWM${a.channel}`).join(' / ')}</p><small>配置摘要 {state.hardware.wiring_hash}</small></div>}
   ...
   <aside className="control-panel">
    ...
    <div className="panel-content">
     {tab==='joint'?<div className="joint-list">{q.map((value,i)=><div className={'joint-row '+(selectedJoint===i?'focused':'')} key={i} onClick={()=>setSelectedJoint(i)}><div className="joint-label"><strong>{i===5?'G':'J'+(i+1)}</strong><label htmlFor={'joint-'+i}>{names[i]}</label><input id={'joint-'+i} aria-label={names[i]+'目标角度'} type="number" step=".5" min={(state.effective_limits_deg??boot.model.limits_deg)[i][0]} max={(state.effective_limits_deg??boot.model.limits_deg)[i][1]} value={Number(value.toFixed(2))} disabled={disabled} onChange={e=>changeQ(i,Number(e.target.value))}/><span>°</span></div><div className="slider-line"><button disabled={disabled} title="目标减小 1 度" onClick={()=>changeQ(i,Math.max((state.effective_limits_deg??boot.model.limits_deg)[i][0],value-1))}>−</button><input aria-label={names[i]+'滑条'} type="range" min={(state.effective_limits_deg??boot.model.limits_deg)[i][0]} max={(state.effective_limits_deg??boot.model.limits_deg)[i][1]} step=".1" value={value} disabled={disabled} onChange={e=>changeQ(i,Number(e.target.value))}/><button disabled={disabled} title="目标增大 1 度" onClick={()=>changeQ(i,Math.min((state.effective_limits_deg??boot.model.limits_deg)[i][1],value+1))}>+</button></div></div>)}</div>:<div className="cartesian">
