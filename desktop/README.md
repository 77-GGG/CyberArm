...
工具栏 → 控制台。默认小浮窗，标题栏可拖动、右下角可拉伸，位置与大小在本次会话内保留。支持 19 个命令、参数帮助、历史、补全和结构化结果。输入 `help` 查看列表，`status` 查询仿真状态，`joint 1 3` 调整底座。命令复用原有运动校验；手动模式仍默认关闭路径显示。
...
完整命令、Python SDK、协议和硬件接入方案见 [控制台与通信接口](../docs/控制台与通信接口_20260915.md)。当前只实现仿真通信，不包含串口固件或真实舵机反馈。
