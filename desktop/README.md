...
固件位于 `src/main.cpp`，目标板是 `esp32-s3-devkitc-1`，默认 PCA9685 地址 `0x40`、SDA GPIO8、SCL GPIO9、通道 0–5 对应 J1–J5/G。依赖和 USB CDC 编译开关见 `platformio.ini`。完整接线、回中、校准、命令和已知限制见 [实机同步控制](../docs/实机同步控制_20260916.md)。普通三线舵机没有位置回读，软件显示的是指令角；实测角仍为空。

## 0.4.0 命令控制台

工具栏 → 控制台。默认小浮窗，标题栏可拖动、右下角可拉伸，位置与大小在本次会话内保留。支持 27 个命令、参数帮助、历史、补全和结构化结果。输入 `help` 查看列表，`status` 查询状态，`joint 1 3` 调整底座。命令复用原有运动校验；手动模式仍默认关闭路径显示。
