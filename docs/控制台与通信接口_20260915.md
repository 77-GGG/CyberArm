...
命令窗口与三维视图共用同一个模拟控制器。回车执行，上下键查看本次会话最近 100 条历史，Tab 补全唯一命令，`clear` 清屏。关闭应用不保存历史。底部停止和 Esc 始终可用。操作说明默认折叠，可点击“仿真 · ° / mm · help 查看命令”展开。

当前状态明确标注 `state_source: simulation`；`device` 显示 ESP32-S3、K230D 和舵机均未连接。不能从仿真角度推断真实舵机位置。
...
SDK `Transport` 定义 `get/post/close`，默认 `HttpTransport`。替代实现可用于协议模拟和测试；真实串口应由后端硬件适配器统一持有，避免 UI 与 CLI 同时争抢串口。当前没有已实现的 `SerialTransport` 或硬件连接切换。
